...
const CustomToggle = React.forwardRef<HTMLDivElement, any>(({ children, className, ...props }, ref) => (
    <div
        className={cn(
            'inline-flex h-11 p-1 items-center justify-center rounded-full bg-black',
            className
        )}
        ref={ref}
        {...props}
...
const CustomToggleItem = React.forwardRef<HTMLButtonElement, any>(({ children, className, ...props }, ref) => (
    <button
        className={cn(
            'flex grow items-center justify-center rounded-full text-sm font-medium transition-all px-8 py-2',
            'text-slate-300',
