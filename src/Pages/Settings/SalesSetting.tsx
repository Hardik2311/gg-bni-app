...
const SalesSettingsPage: React.FC = () => {
    ...
    const settingsDocRef = doc(db, 'settings', 'sales-settings');
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    setSettings({
                        ...
                        voucherName: 'Main Sales',
                        voucherPrefix: 'SLS-',
                        currentVoucherNumber: 0,
                    });
                }
            } catch (err) {
                console.error('Failed to fetch sales settings:', err);
        ...
        <div className="flex flex-col min-h-screen bg-white w-full">
            ...
            <main className="flex-grow p-4 bg-gray-50 w-full overflow-y-auto box-border">
                <form onSubmit={handleSave} className="bg-white rounded-lg p-6 shadow-md">
                    ...
                    {/* Checkboxes from previous response */}
                    <div className="flex items-center mb-4">
                        <input type="checkbox" id="required-customer" checked={settings.requiredCustomerName || false} onChange={(e) => setSettings({ ...settings, requiredCustomerName: e.target.checked })} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                        <label htmlFor="required-customer" className="ml-2 text-gray-700 text-sm font-medium">Require Party Name for Sale</label>
                    </div>
                    <div className="flex items-center mb-4">
                        <input type="checkbox" id="item-discount" checked={settings.itemWiseDiscount || false} onChange={(e) => setSettings({ ...settings, itemWiseDiscount: e.target.checked })} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                        <label htmlFor="item-discount" className="ml-2 text-gray-700 text-sm font-medium">Enable Item-wise Discount</label>
                    </div>
                    <div className="flex items-center mb-4">
                        <input type="checkbox" id="enforce-mrp" checked={settings.enforceMRP || false} onChange={(e) => setSettings({ ...settings, enforceMRP: e.target.checked })} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                        <label htmlFor="enforce-mrp" className="ml-2 text-gray-700 text-sm font-medium">Enforce Selling Price &gt;= MRP</label>
                    </div>
                    <div className="flex items-center mb-4">
                        <input type="checkbox" id="print-barcode" checked={settings.enableBarcodePrinting || false} onChange={(e) => setSettings({ ...settings, enableBarcodePrinting: e.target.checked })} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                        <label htmlFor="print-barcode" className="ml-2 text-gray-700 text-sm font-medium">Enable Barcode Printing</label>
                    </div>
                    <div className="flex items-center mb-6">
                        <input type="checkbox" id="copy-voucher" checked={settings.copyVoucherAfterSaving || false} onChange={(e) => setSettings({ ...settings, copyVoucherAfterSaving: e.target.checked })} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                        <label htmlFor="copy-voucher" className="ml-2 text-gray-700 text-sm font-medium">Copy Sales Voucher After Saving</label>
                    </div>
